...
type RequestBodyInterface =
  ...
  | FirebaseUserRequestBody
  | FirebaseCompanyRequestBody
...
export default function makeFirebaseRequest(
  ...
  switch (targetEndpoint) {
    ...
    case "auth/signUp/company":
      if (
        isFirebaseAuthBody(body) &&
        "companyName" in body &&
        "companyAvatar" in body
      ) {
    ...
    case "user/create":
      if (
        isFirebaseUserBody(body) &&
        "displayName" in body &&
        "email" in body &&
        "isCompanyOwner" in body
      ) {
    ...
    case "company/create":
      if (
        ...
        "name" in body &&
        "avatarPath" in body &&
        "initialUserEmail" in body
      ) {
        createNewCompany(
          body.companyId,
          body.name!,
          body.avatarPath!,
          body.initialUserEmail!,
        );
      }
      break;
    case "company/get":
