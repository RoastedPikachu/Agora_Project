...
const ProfileModalWindow = () => {
  ...
  const changeUserStatus = (statusCode: number) => {
    makeFirebaseRequest("user/update/status", {
      userId: auth.currentUser?.uid,
  ...
  return (
    <div className="absolute top-[51px] right-0 pt-[20px] pb-[5px] w-[300px] h-auto bg-[#ffffff] border-2 border-[#e5e8eb] rounded-[10px] z-30">
        ...
        <div className="grid items-center grid-rows-2 grid-cols-1 ml-[15px]">
          ...
          <div className="flex items-center">
            ...
            <p className="ml-[10px] text-[#4f7396] text-[1rem] font-normal">
              {statuses.find((status) => status.isCurrent)?.name}
            </p>
          </div>
        </div>
      </div>
      ...
      <button
        onClick={() => changeUserStatus(20)}
        className="mx-[25px] h-[50px] bg-transparent border-none text-[#4f7396] text-left font-medium"
      >
        Set yourself as <b>not available</b>
      </button>

      <button
        onClick={() => modalWindowsStore.changeInviteCodeModalOpened()}
        className="mx-[25px] h-[40px] bg-transparent border-none text-[#4f7396] text-left font-medium"
      >
        Generate <b>invite code</b>
      </button>
      ...
      <label
        htmlFor="UserAvatar"
        className="flex items-center mx-[25px] h-[50px] bg-transparent border-none text-[#4f7396] text-left font-semibold cursor-pointer"
      >
        Change avatar
      </label>
      ...
      <input
        type="file"
        onChange={(event) => handleImageLoad(event, changeUserAvatar)}
        id="CompanyAvatar"
        className="hidden"
      />
      ...
      <button
        onClick={() => signOut()}
        className="px-[25px] w-full h-[50px] bg-transparent border-t-2 border-[#e5e8eb] text-[#4f7396] text-left font-medium"
      >
        Sign out of Agora
      </button>
    </div>
